...
class main:
    ...
    def loadFiles(self):
        try:
            ...
            #     with open("ApiInfo.json", "r") as js:
            #         self.jsonDict = json.load(fp=js)
            # else:
            ...
            try:
                with open("httpRequest.txt", "a") as f:
                    if request.method == "POST":
                        ...
                        if len(postARG) and (
                            httpPostString not in self.httpRequestlist
                        ):
                            self.httpRequestlist.append(httpPostString)
                            if len(postARG[0]) != 0:
                                f.write(
                                    httpPostString + "(" + request.method + ")" + "\n"
                                )
                                ...
                                for i in postARG:
                                    ...
                                    #     json.dump(self.jsonDict, fp=js)
                                    # js.write(json.dumps(str(self.jsonDict)) + "\n")
